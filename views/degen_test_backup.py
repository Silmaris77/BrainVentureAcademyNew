...
def show_degen_test():
    ...
    if st.session_state.show_test_info:        st.markdown("""
        ### 👉 Jak działa ten test?
        ...
        if zen_button("Rozpocznij test"):
            ...
            st.rerun()
          # Opcja przeglądania typów degenów        st.markdown("---")        st.markdown("### 📚 Chcesz dowiedzieć się więcej o typach neuroleaderów?")
        ...
        if selected_type:
            ...
            if device_type == 'mobile':
                # Na telefonach wyświetl sekcje jedna pod drugą                content_section("Mocne strony:", 
                ...
                with col1:                    content_section("Mocne strony:", 
                                   "\n".join([f"- ✅ {strength}" for strength in NEUROLEADER_TYPES[selected_type]["strengths"]]), 
                                   icon="💪", 
                                   collapsed=False)
            ...
            if st.checkbox("Pokaż szczegółowy opis typu"):
                if selected_type in degen_details:
                    ...
                    st.warning("Szczegółowy opis dla tego typu neuroleadera nie jest jeszcze dostępny.")    # Tryb testu    
