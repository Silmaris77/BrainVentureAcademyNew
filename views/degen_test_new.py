# filepath: c:\Users\pksia\Dropbox\Brainventure - kurs\B2\BrainVentureAcademyNew\views\degen_test.py
...
def plot_radar_chart(scores, device_type=None):
    ...
    if device_type == 'mobile':
        ...
        try:
            ...
            # Fallback if set_thetagrids is not available
            pass
    else:
        try:
    ...
    for i, (angle, value) in enumerate(zip(angles_radians, values)):
        if labels[i] in NEUROLEADER_TYPES and "color" in NEUROLEADER_TYPES[labels[i]]:
...
def show_degen_test():
    ...
    if st.session_state.show_test_info:
        ...
        # Opcja przeglądania typów neuroleaderów
        st.markdown("---")
        st.markdown("### 📚 Chcesz dowiedzieć się więcej o typach neuroleaderów?")
        selected_type = st.selectbox("Wybierz typ neuroleadera:", list(NEUROLEADER_TYPES.keys()))
        ...
        if selected_type:
            ...
            # Dodajemy szczegółowy opis typu neuroleadera
    ...
    # Tryb testu    
        ...
        # Update user data
        users_data = load_user_data()        users_data[st.session_state.username]["degen_type"] = dominant_type
