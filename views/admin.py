...
def get_degen_type_distribution():
    """Analizuje rozkład typów neuroleaderów wśród użytkowników"""
    users_data = load_user_data()
    
    # Zlicz wystąpienia każdego typu neuroleadera
    neuroleader_counts = {}
    for _, data in users_data.items():
        neuroleader_type = data.get('degen_type', 'Nieznany')  # Zachowujemy nazwę klucza dla kompatybilności
...
def show_admin_dashboard():
    ...
    with admin_tabs[0]:
        ...
        st.altair_chart(chart, use_container_width=True)
          # Rozkład typów neuroleaderów
        st.subheader("Rozkład typów neuroleaderów")
        
        neuroleader_df = get_degen_type_distribution()
        
        if not neuroleader_df.empty:
            # Wykres kołowy typów neuroleaderów
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.pie(neuroleader_df['count'], labels=neuroleader_df['neuroleader_type'], autopct='%1.1f%%', 
